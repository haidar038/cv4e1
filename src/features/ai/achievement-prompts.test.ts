...
import {
  ...
  getAchievementGroundingRules,
  getAchievementPromptBody,
...
describe('achievement prompts (unified flow, FR-404/FR-405)', () => {
  it('pins the active prompt version', () => {
    expect(ACHIEVEMENT_PROMPT_VERSION).toBe('v1')
  })
  ...
  it('composes grounding rules above the capability body', () => {
    ...
    expect(systemPrompt).toContain(getAchievementGroundingRules())
    expect(systemPrompt).toContain(getAchievementPromptBody())
    expect(systemPrompt.indexOf(getAchievementGroundingRules())).toBeLessThan(
      systemPrompt.indexOf(getAchievementPromptBody()),
    )
  })

  it('mirrors every normative grounding sentence in the capability prompt', () => {
    const body = getAchievementPromptBody()
    for (const sentence of MIRRORED_NORMATIVES) {
      expect(getAchievementGroundingRules()).toContain(sentence)
  ...
  it('documents the token budgets the orchestrator enforces', () => {
    ...
    expect(ACHIEVEMENT_MAX_COMPLETION_TOKENS).toBe(800)
    expect(ACHIEVEMENT_MAX_SUGGESTIONS).toBe(3)
    expect(getAchievementPromptBody()).toContain('2000')
    expect(getAchievementPromptBody()).toContain('800')
  })
  ...
  it('forbids verb stacking in the capability wording', () => {
    expect(getAchievementPromptBody()).toContain('tanpa menumpuk dua kata kerja')
  })
...
describe('truncateAchievementText', () => {
  it('trims short text untouched', () => {
    expect(truncateAchievementText('  membantu acara kampus  ', 2000)).toBe('membantu acara kampus')
    expect(truncateAchievementText('', 2000)).toBe('')
  })

  it('marks truncation inside a bracket span the grounding check strips', () => {
    const long = 'a'.repeat(2100)
    const cut = truncateAchievementText(long, 2000)
